...
# Function to check win status or returns 0 if game is not over
...
# Function to display game statstics

def game_stats(t1, t2, moves_count, attempts):
    print("\nGame stats:")
    print("Number of valid moves made for current board : ", moves_count)
    print("Number of presses throughout the current session : ", attempts)
    seconds_spent = t2-t1
    minutes_spent = 0
    if seconds_spent > 60:
...
def play_game():
    ...
    print("\nEnter any key to continue to the game\n")
    ...
    while result == 0:
        ...
        if moves_count==0 and player_choice!='Z' and attempts==0:
            ...
            print("\n1. Enter move\n2. Enter x to stop the game\n3. Enter r to clear board\n4. Enter g to restart entire game\n")
            t1 = time.perf_counter()
        player_choice = input()
        ...
        if player_choice in valid_moves:
            ...
            print("\nAre you sure you want to restart entire game ?\nPress y for yes or any other key to continue game\n")
            player_choice = input()
            if player_choice=='y' or player_choice=='Y':
                print("\nYou have restarted the game\n")
                play_game()
                game_restart_check = 1
                break
    ...
    if game_restart_check == 0:
        game_stats(t1, t2, moves_count, attempts)
